...
const YouMayAlsoLike = ({ currentSlug }) => {
  ...
  const otherProjects = corporatePhotographyProjects.filter(
      ...
      <div className="max-w-7xl mx-auto border-t border-gray-200 pt-12 md:pt-16">
        <div className="flex items-end justify-between gap-6 mb-8 md:mb-10">
          <div>
            ...
            <p className="mt-3 text-black text-base sm:text-lg leading-relaxed">
              Explore more corporate photography from our portfolio.
            </p>
          </div>
        ...
        >
          {otherProjects.map((item) => (
            <Link
              key={item.slug}
              href={`/corporate/photography/${item.slug}`}
...
const CorporatePhotographyDetail = ({ project }) => {
              ...
              {gallery.map((image, index) => (
                ...
                >
                  <img
                    ...
                    className="w-full h-auto transition duration-700 ease-out group-hover:scale-[1.03]"
                  />
...
export default CorporatePhotographyDetail;
